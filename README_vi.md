...
- **AI Endpoints (OpenAI Gym Style):** Lấy observation game có cấu trúc (`/observation`), gửi hành động (`/action`), step kết hợp (`/step`), stream SSE (`/stream`).
- **Screen Observation:** Phát hiện màn hình UI, component (button, slider, textbox) và navigation path.
- **AI Agent Guide:** Tài liệu hướng dẫn AI hiểu observation và làm chủ Minecraft. Xem [docs/vi/AI_AGENT_GUIDE.md](docs/vi/AI_AGENT_GUIDE.md).
...
**Stream Observation theo thời gian thực (SSE):**
```bash
